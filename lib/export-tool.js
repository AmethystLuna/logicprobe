...
import { exportModel } from './exporters.js';
...
export const logicProbeExportTool = defineTool({
    ...
    description: 'Export a LogicModelV1 state machine into native input for an external verification tool (logicprobe routing). format is one of uppaal (.xta + queries), tla (TLC spec + safety property), prism (.pm + .pctl), spin (Promela + ltl). Booleans are exported as 0/1 integers; invariants the target cannot express are returned as warnings, never silently dropped. Returns { ok, format, primary, extras?, warnings? } or { ok: false, error } when the model is invalid or the PRISM enumeration is too large.',
    parameters: {
        model: {
            type: 'json',
            required: true,
    ...
    async execute(args) {
        try {
            const result = exportModel(args.model, args.format);
            return { ok: true, format: result.format, primary: result.primary, ...(result.extras === undefined ? {} : { extras: result.extras }), warnings: result.warnings };
        }
        catch (error) {
