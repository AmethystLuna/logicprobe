...
function validateField(input, path, errors, fieldNames) {
    ...
    if (input.monotonic !== undefined && input.monotonic !== 'inc' && input.monotonic !== 'dec')
        bad(errors, path + '.monotonic', "must be 'inc' or 'dec'");
        ...
        if (!Array.isArray(root.invariants))
            ...
            root.invariants.forEach((entry, index) => {
                ...
                if (kind === 'field-required' || kind === 'no-orphan') {
                    ...
                    if (typeof entry.field !== 'string')
                        bad(errors, path + '.field', 'must be a string');
                    if (entry.direction !== 'inc' && entry.direction !== 'dec')
                        bad(errors, path + '.direction', "must be 'inc' or 'dec'");
                }
                else if (kind === 'sequence') {
                    if (!Array.isArray(entry.steps) || entry.steps.length === 0)
                        bad(errors, path + '.steps', 'must be a non-empty array');
                    else
                        entry.steps.forEach((step, stepIndex) => {
                            if (typeof step !== 'string' || step.length === 0)
                                bad(errors, path + '.steps[' + stepIndex + ']', 'must be a non-empty string');
                        });
                }
...
function dataStepRegistry(options) {
    const map = new Map();
    for (const pair of options.copyPairs ?? [])
        map.set(pair.id, { kind: 'copy', copyPair: pair });
...
const MONOTONIC_DATA_TYPES = new Set(['integer', 'number', 'date', 'datetime', 'timestamp']);
function DA9_dataMonotonic(model, options) {
    const findings = [];
    for (const invariant of model.invariants ?? []) {
        if (invariant.kind !== 'monotonic')
            continue;
...
function DA10_dataSequence(model, options) {
    const findings = [];
...
function DA11_dataLeadsTo(model, options) {
    const findings = [];
    for (const invariant of model.invariants ?? []) {
        ...
        if (field.enum !== undefined && field.enum.length > 0) {
            if (!field.enum.includes(invariant.from))
                findings.push({ code: 'DA11_FROM_NOT_IN_ENUM', severity: 'warning', message: 'Leads-to from value ' + String(invariant.from) + ' is not in enum for ' + invariant.entity + '.' + invariant.field, evidence: { invariant } });
            if (!field.enum.includes(invariant.to))
                findings.push({ code: 'DA11_TO_NOT_IN_ENUM', severity: 'warning', message: 'Leads-to to value ' + String(invariant.to) + ' is not in enum for ' + invariant.entity + '.' + invariant.field, evidence: { invariant } });
        }
    }
...
function DA12_dataAtomicity(model, options) {
    const findings = [];
    const registry = dataStepRegistry(options);
    for (const invariant of model.invariants ?? []) {
        ...
        for (const step of invariant.steps) {
            ...
            if (entry.kind === 'copy' && entry.copyPair !== undefined) {
                ...
                if (!hasBackup)
                    findings.push({ code: 'DA12_ATOMIC_COPY_NO_BACKUP', severity: 'warning', message: 'Atomic copy step ' + step + ' has no backup/restore pair', evidence: { invariant, step } });
            }
        }
    }
