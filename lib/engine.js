...
export function runCompositionVerification(machinesInput, options = {}) {
    ...
    const compositionMoves = (node) => {
        const moves = [];
        const machines = [node.r1, node.r2];
        for (let index = 0; index < 2; index++) {
            const model = models[index];
            const runtime = machines[index];
            if (isTerminal(model, runtime.state))
                continue;
            for (const event of allEvents(model)) {
        ...
        for (const event of rendezvous) {
            if (isTerminal(models[0], node.r1.state) || isTerminal(models[1], node.r2.state))
                continue;
    ...
    const keyOf = (node) => runtimeKey(node.r1) + '|' + runtimeKey(node.r2);
    const init = { r1: initialState(models[0]), r2: initialState(models[1]), path: [] };
    const visited = new Set();
    const queue = [init];
    ...
    const c2Findings = [];
    const machineEvents = new Set();
    for (const model of models)
        for (const transition of model.transitions)
            machineEvents.add(transition.event);
    for (const event of rendezvous) {
        if (!machineEvents.has(event))
            continue;
