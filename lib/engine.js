...
export function validateModel(input) {
    ...
    if (!Array.isArray(root.transitions)) {
        ...
        root.transitions.forEach((entry, index) => {
            ...
            if (transition.weight !== undefined) {
                if (typeof transition.weight !== 'number' || !Number.isFinite(transition.weight) || transition.weight < 0)
                    bad(path + '.weight', 'must be a non-negative finite number');
            }
        });
    }
    if (Array.isArray(root.states)) {
        root.states.forEach((state, index) => {
            ...
            root.invariants.forEach((entry, index) => {
                ...
                if (invariant.kind === 'never-states') {
                    ...
                    if (typeof invariant.p !== 'number' || !Number.isFinite(invariant.p) || invariant.p < 0 || invariant.p > 1)
                        bad(path + '.p', 'must be a number in [0, 1]');
                }
                else {
    ...
    if (root.tickEvents !== undefined) {
        if (!Array.isArray(root.tickEvents))
            bad('tickEvents', 'must be an array');
        else
            root.tickEvents.forEach((entry, index) => {
                if (typeof entry !== 'string' || entry.length === 0)
                    bad('tickEvents[' + index + ']', 'must be a non-empty string');
            });
    }
...
function runtimeGraph(model, options) {
...
function runtimePathToKey(graph, targetKey) {
    const parent = new Map();
    const visited = new Set([graph.initKey]);
    const queue = [graph.initKey];
    while (queue.length > 0) {
        const current = queue.shift();
...
function keyOfFromStep(graph, step, targetKey) {
...
function findUnboundedCycle(model, options) {
...
// ---------------------------------------------------------------------------
function probabilityOutcomes(model, runtime) {
    const groupMap = groupTransitions(model);
    const outcomes = new Map();
    for (const event of allEvents(model)) {
        const group = groupMap.get(runtime.state + '|' + event);
        ...
        for (const transition of applicableTransitions(group, runtime)) {
            ...
            if (current === undefined)
                outcomes.set(key, { state: next.state, weight });
            else
 ...
 */
function computeHitProbability(model, options, targetState) {
    const exploration = explore(model, options);
    ...
    const fixedValue = new Array(n).fill(null);
    const chains = [];
    for (let i = 0; i < n; i++) {
...
function A13_probability(model, options) {
    const findings = [];
    const eps = 1e-9;
    for (const invariant of model.invariants ?? []) {
        ...
        let violated = false;
        if (invariant.op === '>=')
            violated = probability < invariant.p - eps;
        else if (invariant.op === '>')
            violated = probability <= invariant.p + eps;
        else if (invariant.op === '<=')
            violated = probability > invariant.p + eps;
        else
...
// ---------------------------------------------------------------------------
function A14_deadline(model, options) {
    const findings = [];
    ...
    const groupMap = groupTransitions(model);
    const init = initialState(model);
    const limitOf = (stateId) => limits.get(stateId);
    const queue = [{ runtime: init, ticks: 0, path: [] }];
    const seen = new Set();
    let steps = 0;
    while (queue.length > 0) {
        const entry = queue.shift();
        ...
        for (const event of allEvents(model)) {
            ...
            for (const transition of applicableTransitions(group, entry.runtime)) {
                ...
                if (!seen.has(nextKey))
                    queue.push({ runtime: next, ticks, path });
            }
        }
    }
 ...
 */
export function runCompositionVerification(machinesInput, options = {}) {
    const models = [];
    const hashes = [];
    const modelFindings = [];
    ...
    const compositionMoves = (node) => {
        const moves = [];
        // single-machine (non-rendezvous) steps
        for (let i = 0; i < models.length; i++) {
            if (isTerminal(models[i], node.runtimes[i].state))
                continue;
            for (const event of allEvents(models[i])) {
                if (rendezvous.has(event))
                    continue;
        ...
        for (const event of rendezvous) {
            ...
            for (let i = 0; i < models.length; i++) {
                if (isTerminal(models[i], node.runtimes[i].state))
                    continue;
                if (!machineEventSets[i].has(event))
                    continue;
                participants.push(i);
            }
            if (participants.length < 2)
                continue; // handshake needs a partner
            const outcomes = participants.map((i) => stepRuntime(models[i], node.runtimes[i], event));
            if (outcomes.some((list) => list.length === 0))
                continue; // not all participants ready
            let combos = [node.runtimes.slice()];
    ...
    const keyOf = (node) => node.runtimes.map((runtime) => runtimeKey(runtime)).join('|');
    const init = { runtimes: models.map((model) => initialState(model)), path: [] };
    const visited = new Set();
    const queue = [init];
    ...
    while (queue.length > 0) {
        ...
        for (const move of moves) {
            if (move.machines.length > 1)
                firedCount.set(move.event, (firedCount.get(move.event) ?? 0) + 1);
    ...
    for (const set of machineEventSets)
        for (const event of set)
            allEventsSet.add(event);
    for (const event of rendezvous) {
        if (!allEventsSet.has(event))
            continue;
