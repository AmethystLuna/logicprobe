...
function exportUppaal(ex: ExportedModel): ExportResult {
  ...
  const safeEvent = (event: string): string => xml(event.replace(/[^A-Za-z0-9_]/g, '_'))
  ...
  const edges = model.transitions.map((tr, i) => {
    ...
    const labels: string[] = []
    if (guard !== '') labels.push('      <label kind=\"guard\" x=\"16\" y=\"16\">' + xml(guard) + '</label>')
    if (assigns.length > 0) labels.push('      <label kind=\"assignment\" x=\"16\" y=\"16\">' + xml(assigns.join(',\n')) + '</label>')
  ...
  for (const state of model.states) if (state.terminal) queries.push('E<> LogicProbe.' + stateIdOf(ex, state.id))
  ...
  for (const invariant of model.invariants ?? []) {
    if (invariant.kind !== 'never-states') warnings.push('UPPAAL export carries invariant kind ' + invariant.kind + ' only as a comment (not expressible in the exported XML).')
  }
...
function stateIdOf(ex: ExportedModel, id: string): string {
