...
export type DataInvariantSpec =
  | { id: string; description: string; kind: 'field-required'; entity: string; field: string }
  | { id: string; description: string; kind: 'unique'; entity: string; fields: string[] }
  | { id: string; description: string; kind: 'referential-integrity'; entity: string; field: string; refEntity: string; refField?: string }
  | { id: string; description: string; kind: 'range'; entity: string; field: string; min?: number; max?: number }
...
export function dataModelHash(model: DataModelV1): string {
  return createHash('sha256').update(stableStringify(model)).digest('hex')
}

function bad(errors: string[], path: string, message: string): void {
...
function validateField(input: unknown, path: string, errors: string[], fieldNames: Set<string>): FieldSpec | undefined {
  ...
  if (input.required !== undefined && typeof input.required !== 'boolean') bad(errors, path + '.required', 'must be a boolean')
  if (input.unique !== undefined && typeof input.unique !== 'boolean') bad(errors, path + '.unique', 'must be a boolean')
  if (input.nullable !== undefined && typeof input.nullable !== 'boolean') bad(errors, path + '.nullable', 'must be a boolean')
  if (input.min !== undefined && typeof input.min !== 'number') bad(errors, path + '.min', 'must be a number')
  ...
  if (input.items !== undefined) validateField(input.items, path + '.items', errors, new Set<string>())
  return input as unknown as FieldSpec
}

function entityByName(model: DataModelV1, name: string): EntitySpec | undefined {
  return model.entities.find((entity) => entity.name === name)
}

function fieldByPath(model: DataModelV1, entityName: string, fieldName: string): FieldSpec | undefined {
  return entityByName(model, entityName)?.fields.find((field) => field.name === fieldName)
}

function mapFieldPath(mapping: Record<string, string>, path: string): string {
  return mapping[path] ?? path
}

export function validateDataModel(input: unknown): { ok: true; model: DataModelV1 } | { ok: false; errors: string[] } {
  const errors: string[] = []
  if (!isPlainObject(input)) {
    return { ok: false, errors: ['model: must be an object'] }
  }
  const root = input as Record<string, unknown>
  if (root.schemaVersion !== 1) bad(errors, 'schemaVersion', 'must be 1')
  if (!Array.isArray(root.entities) || root.entities.length === 0) {
    ...
    root.entities.forEach((entry, index) => {
      ...
      if (entry.primaryKey !== undefined) {
        if (!Array.isArray(entry.primaryKey)) bad(errors, path + '.primaryKey', 'must be an array')
      }
      if (entry.uniqueKeys !== undefined) {
        if (!Array.isArray(entry.uniqueKeys)) bad(errors, path + '.uniqueKeys', 'must be an array')
      }
      if (entry.indexes !== undefined) {
        if (!Array.isArray(entry.indexes)) bad(errors, path + '.indexes', 'must be an array')
      }
    })
  }
  if (root.relationships !== undefined) {
    if (!Array.isArray(root.relationships)) bad(errors, 'relationships', 'must be an array')
  }
  if (root.invariants !== undefined) {
    if (!Array.isArray(root.invariants)) bad(errors, 'invariants', 'must be an array')
      ...
      if (typeof entry.id !== 'string' || entry.id.length === 0) bad(errors, path + '.id', 'must be a non-empty string')
      if (typeof entry.description !== 'string') bad(errors, path + '.description', 'must be a string')
      ...
      if (kind === 'field-required' || kind === 'no-orphan') {
        ...
        if (!Array.isArray(entry.fields)) bad(errors, path + '.fields', 'must be an array')
      } else if (kind === 'referential-integrity') {
        if (typeof entry.entity !== 'string') bad(errors, path + '.entity', 'must be a string')
        if (typeof entry.field !== 'string') bad(errors, path + '.field', 'must be a string')
        if (typeof entry.refEntity !== 'string') bad(errors, path + '.refEntity', 'must be a string')
        if (entry.refField !== undefined && typeof entry.refField !== 'string') bad(errors, path + '.refField', 'must be a string')
      } else if (kind === 'range') {
        if (typeof entry.entity !== 'string') bad(errors, path + '.entity', 'must be a string')
        if (typeof entry.field !== 'string') bad(errors, path + '.field', 'must be a string')
        if (entry.min !== undefined && typeof entry.min !== 'number') bad(errors, path + '.min', 'must be a number')
        if (entry.max !== undefined && typeof entry.max !== 'number') bad(errors, path + '.max', 'must be a number')
  ...
  if (root.boundaryChecks !== undefined) {
    if (!Array.isArray(root.boundaryChecks)) bad(errors, 'boundaryChecks', 'must be an array')
  }
  if (errors.length > 0) return { ok: false, errors }
  return { ok: true, model: root as unknown as DataModelV1 }
}
...
function allFieldPaths(model: DataModelV1): string[] {
  const result: string[] = []
  for (const entity of model.entities) {
    for (const field of entity.fields) result.push(entity.name + '.' + field.name)
  }
  return result
}

function entityFieldMap(model: DataModelV1): Map<string, FieldSpec> {
  const map = new Map<string, FieldSpec>()
  for (const entity of model.entities) {
    for (const field of entity.fields) map.set(entity.name + '.' + field.name, field)
  }
  return map
}

function DS1_schemaWellFormed(model: DataModelV1): CheckResult {
  ...
  for (const entity of model.entities) {
    ...
    for (const key of entity.primaryKey ?? []) {
      if (!fieldNames.has(key)) findings.push({ code: 'DS1_PRIMARY_KEY_MISSING', severity: 'error', message: 'Primary key field ' + key + ' does not exist in entity ' + entity.name, evidence: { entity: entity.name, field: key } })
    }
    for (const keySet of entity.uniqueKeys ?? []) {
      for (const key of keySet) {
        if (!fieldNames.has(key)) findings.push({ code: 'DS1_UNIQUE_KEY_MISSING', severity: 'error', message: 'Unique key field ' + key + ' does not exist in entity ' + entity.name, evidence: { entity: entity.name, field: key } })
      }
    }
    for (const keySet of entity.indexes ?? []) {
      for (const key of keySet) {
        if (!fieldNames.has(key)) findings.push({ code: 'DS1_INDEX_MISSING', severity: 'error', message: 'Index field ' + key + ' does not exist in entity ' + entity.name, evidence: { entity: entity.name, field: key } })
      }
    }
  }
...
function DS2_requiredCompleteness(model: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
...
function DS3_relationshipIntegrity(model: DataModelV1): CheckResult {
  const findings: Finding[] = []
...
function DS4_typeNullabilityConsistency(model: DataModelV1): CheckResult {
  const findings: Finding[] = []
  for (const entity of model.entities) {
    for (const field of entity.fields) {
...
function DA1_nullEmptyInjection(model: DataModelV1): CheckResult {
  const findings: Finding[] = []
  for (const entity of model.entities) {
    for (const field of entity.fields) {
      if (field.required === true && field.nullable === true) {
...
function DA2_boundaryBlast(model: DataModelV1): CheckResult {
  const findings: Finding[] = []
  for (const entity of model.entities) {
    for (const field of entity.fields) {
      if (typeof field.min === 'number' && typeof field.max === 'number' && field.min > field.max) continue
      if (typeof field.default === 'number') {
        if (field.min !== undefined && field.default < field.min) findings.push({ code: 'DA2_DEFAULT_BELOW_MIN', severity: 'error', message: 'Default ' + field.default + ' is below min ' + field.min + ' for ' + entity.name + '.' + field.name, evidence: { entity: entity.name, field: field.name } })
        if (field.max !== undefined && field.default > field.max) findings.push({ code: 'DA2_DEFAULT_ABOVE_MAX', severity: 'error', message: 'Default ' + field.default + ' is above max ' + field.max + ' for ' + entity.name + '.' + field.name, evidence: { entity: entity.name, field: field.name } })
      }
      if (field.enum !== undefined && field.enum.length > 0) {
        if (typeof field.min === 'number') {
          for (const value of field.enum) {
            if (typeof value === 'number' && value < field.min) findings.push({ code: 'DA2_ENUM_BELOW_MIN', severity: 'warning', message: 'Enum value ' + value + ' is below min ' + field.min + ' for ' + entity.name + '.' + field.name, evidence: { entity: entity.name, field: field.name, value } })
          }
        }
        if (typeof field.max === 'number') {
          for (const value of field.enum) {
            if (typeof value === 'number' && value > field.max) findings.push({ code: 'DA2_ENUM_ABOVE_MAX', severity: 'warning', message: 'Enum value ' + value + ' is above max ' + field.max + ' for ' + entity.name + '.' + field.name, evidence: { entity: entity.name, field: field.name, value } })
          }
        }
      }
    }
  ...
  for (const check of model.boundaryChecks ?? []) {
    ...
    for (const value of check.values ?? []) {
      if (typeof value === 'number') {
        if (field.min !== undefined && value < field.min) findings.push({ code: 'DA2_BOUNDARY_BELOW_MIN', severity: 'warning', message: 'Boundary value ' + value + ' below min ' + field.min + ' for ' + check.entity + '.' + check.field, evidence: { entity: check.entity, field: check.field, value } })
        if (field.max !== undefined && value > field.max) findings.push({ code: 'DA2_BOUNDARY_ABOVE_MAX', severity: 'warning', message: 'Boundary value ' + value + ' above max ' + field.max + ' for ' + check.entity + '.' + check.field, evidence: { entity: check.entity, field: check.field, value } })
      }
...
function DA3_uniquenessViolation(model: DataModelV1): CheckResult {
  const findings: Finding[] = []
  for (const entity of model.entities) {
    for (const key of entity.primaryKey ?? []) {
      const field = fieldByPath(model, entity.name, key)
      if (field?.nullable === true) findings.push({ code: 'DA3_NULLABLE_PRIMARY_KEY', severity: 'error', message: 'Primary key ' + entity.name + '.' + key + ' is nullable', evidence: { entity: entity.name, field: key } })
    }
    for (const keySet of entity.uniqueKeys ?? []) {
      for (const key of keySet) {
        const field = fieldByPath(model, entity.name, key)
        if (field?.nullable === true) findings.push({ code: 'DA3_NULLABLE_UNIQUE_KEY', severity: 'warning', message: 'Unique key ' + entity.name + '.' + key + ' is nullable', evidence: { entity: entity.name, field: key } })
      }
    }
  }
...
function DA4_referentialIntegrityViolation(model: DataModelV1): CheckResult {
  const findings: Finding[] = []
  for (const relationship of model.relationships ?? []) {
...
function DA5_migrationCoverage(model: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
  if (options.beforeModel === undefined) {
...
function DA6_copyConsistency(model: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
  const afterFields = entityFieldMap(model)
  for (const pair of options.copyPairs ?? []) {
...
function DA7_rollbackBackupSymmetry(model: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
...
function DD2_dataInvariantContinuity(before: DataModelV1, after: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
  ...
  for (const invariant of before.invariants ?? []) {
    if (invariant.kind === 'field-required' || invariant.kind === 'range' || invariant.kind === 'no-orphan') {
      ...
      if (!afterEntities.has(invariant.sourceEntity)) findings.push({ code: 'DD2_INVARIANT_ENTITY_MISSING', severity: 'error', message: 'BEFORE count-equal invariant "' + invariant.id + '" references missing source entity ' + invariant.sourceEntity, evidence: { invariant } })
      if (!afterEntities.has(invariant.targetEntity)) findings.push({ code: 'DD2_INVARIANT_ENTITY_MISSING', severity: 'error', message: 'BEFORE count-equal invariant "' + invariant.id + '" references missing target entity ' + invariant.targetEntity, evidence: { invariant } })
      ...
      if (!afterFields.has(sourceMapped)) findings.push({ code: 'DD2_INVARIANT_FIELD_MISSING', severity: 'error', message: 'BEFORE field-equal invariant "' + invariant.id + '" references missing source field ' + sourceMapped, evidence: { invariant, sourceMapped } })
      if (!afterFields.has(targetMapped)) findings.push({ code: 'DD2_INVARIANT_FIELD_MISSING', severity: 'error', message: 'BEFORE field-equal invariant "' + invariant.id + '" references missing target field ' + targetMapped, evidence: { invariant, targetMapped } })
    }
  }
...
function DD3_deltaSummary(before: DataModelV1, after: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
  const afterEntities = new Set(after.entities.map((entity) => entity.name))
  ...
  for (const path of beforePaths) {
    const mapped = mapFieldPath(mapping, path)
    if (!afterPaths.has(mapped)) removedFields.push(path)
  }
  for (const path of afterPaths) {
    ...
    if (!found) addedFields.push(path)
  }
  for (const entity of removedEntities) findings.push({ code: 'DD3_REMOVED_ENTITY', severity: 'warning', message: 'BEFORE entity ' + entity + ' is not present in AFTER', evidence: { entity } })
  for (const field of removedFields) findings.push({ code: 'DD3_REMOVED_FIELD', severity: 'warning', message: 'BEFORE field ' + field + ' is not present in AFTER', evidence: { field } })
...
function DD4_breakingChangeRegression(before: DataModelV1, after: DataModelV1, options: DataVerificationOptions): CheckResult {
  const findings: Finding[] = []
  const afterFields = entityFieldMap(after)
  const mapping = options.fieldMapping ?? {}
  const migrationFrom = new Set((options.migrationMappings ?? []).map((mapping) => mapping.from))
...
function buildDataComparisonSummary(before: DataModelV1, after: DataModelV1, options: DataVerificationOptions): DataComparisonSummary {
  const afterEntities = new Set(after.entities.map((entity) => entity.name))
  const beforeEntities = new Set(before.entities.map((entity) => entity.name))
...
export function runDataVerification(input: unknown, options: DataVerificationOptions = {}): DataVerificationReport {
  ...
  if (options.beforeModel !== undefined) {
    ...
    const before = beforeValidation.model
    checks.push(
      DD1_dataBehaviorPreservation(before, model, options),
      DD2_dataInvariantContinuity(before, model, options),
      DD3_deltaSummary(before, model, options),
      DD4_breakingChangeRegression(before, model, options),
    )
